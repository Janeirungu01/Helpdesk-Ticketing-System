...
function Tickets({ tickets }) {
  const loggedInUser = JSON.parse(localStorage.getItem("user"));
  const [ticketList, setTicketList] = useState(
    tickets.map((ticket) => ({ ...ticket, resolved: false, notes: "" }))
  );
  const [selectedTicket, setSelectedTicket] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-blue-100 bg-opacity-25 flex justify-center items-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <p className="text-gray-600 mb-4">
              Are you sure you want to mark <span className="font-bold">{selectedTicket.subject}</span> as
              resolved?
            </p>
            <textarea
              value={notes}
